...
vault kv put -mount=security-codedesignplus-keyvalue ms-rbac \
...
vault kv get -mount=security-codedesignplus-keyvalue ms-rbac

# Write database configuration
echo "$newlines"
echo "${BLUE}7. Writing database configuration...${NC}"
vault write security-codedesignplus-database/config/db-ms-rbac \
    plugin_name=mongodb-database-plugin \
    allowed_roles="ms-rbac-mongo-role" \
    connection_url="mongodb://{{username}}:{{password}}@mongo:27017/admin?ssl=false" \
    username="admin" \
    password="password"

vault write security-codedesignplus-database/roles/ms-rbac-mongo-role \
    db_name=db-ms-rbac \
    creation_statements='{ "db": "admin", "roles": [{ "role": "readWrite", "db": "db-ms-rbac" }] }' \
    default_ttl="1h" \
    max_ttl="24h"

vault read security-codedesignplus-database/creds/ms-rbac-mongo-role
...
vault write security-codedesignplus-rabbitmq/roles/ms-rbac-rabbitmq-role \
    vhosts='{"/":{"write": ".*", "read": ".*", "configure": ".*"}}'

vault read security-codedesignplus-rabbitmq/creds/ms-rbac-rabbitmq-role