...
5. Run the desired entry point:
   ...
   - For Worker:
      ```bash
      dotnet run --project src/entrypoints/CodeDesignPlus.Net.Microservice.Rbac.AsyncWorker
      ```
...
1. Build the Docker image using the Dockerfile in the REST API entry point:
   ```bash
   docker build -t ms-rbac-rest . -f src/entrypoints/CodeDesignPlus.Net.Microservice.Rbac.Rest/Dockerfile

   docker run -d -p 5000:5000 --network=backend -e ASPNETCORE_ENVIRONMENT=Docker --name ms-rbac-rest ms-rbac-rest
   ```

2. Build the Docker image using the Dockerfile in the gRPC entry point:
   ```bash
   docker build -t ms-rbac-grpc . -f src/entrypoints/CodeDesignPlus.Net.Microservice.Rbac.gRpc/Dockerfile

   docker run -d -p 5001:5001 --network=backend -e ASPNETCORE_ENVIRONMENT=Docker --name ms-rbac-grpc ms-rbac-grpc
   ```

3. Build the Docker image using the Dockerfile in the Worker entry point:
   ```bash
   docker build -t ms-rbac-worker . -f src/entrypoints/CodeDesignPlus.Net.Microservice.Rbac.AsyncWorker/Dockerfile

   docker run -d -p 5002:5002 --network=backend -e ASPNETCORE_ENVIRONMENT=Docker --name ms-rbac-worker ms-rbac-worker
   ```
